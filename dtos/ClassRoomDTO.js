...
function createClassRoomDTO(input) {
    ...
    if(validateString(input.time))
        errMessages.push("trường'time' chưa hợp lệ")
    if (validateObjectId(input.r_subject))
        errMessages.push("trường 'r_subject' chưa hợp lệ")
    if (errMessages.length > 0)
        return { errMessage: errMessages.reduce((total, err) => `${total} ${err} ---`, "") }
    return { data: { name: input.name,period: input.period,time:input.time, r_subject: input.r_subject } }
}
...
function updateClassRoomDTO(id, reqBody) {
    ...
    if (errMessages.length > 0)
        return { errMessage: errMessages.reduce((total, err) => `${total} ${err}---`, "") }
    const data = { id, name: input.name,period: input.period,time:input.time, r_subject: input.r_subject }
    return { data }
}
